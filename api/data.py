...
def get_csv_files() -> list[Path]:
    """Dynamically discover all frisco_<zip>.csv files in BASE_DIR."""
    return sorted(BASE_DIR.glob("frisco_*.csv"))
...
def get_dataframe() -> pd.DataFrame:
    frames = [pd.read_csv(p, dtype=str).fillna("") for p in get_csv_files()]
    if not frames:
...
def reload_dataframe() -> pd.DataFrame:
    """Clear the cache and reload all CSVs (used after a new ZIP is downloaded)."""
